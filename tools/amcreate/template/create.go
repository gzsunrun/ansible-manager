...
func Create(root string) error {
	...
	for _, v := range am {
		...
		// read tag.yml
		_, err = os.Stat(root + "/tag.yml")
		if err == nil || os.IsExist(err) {
			tagB, err := ioutil.ReadFile(root + "/tag.yml")
			if err != nil {
...
var tag = `- tag_name: 默认
  tag_value: ""
`
var inventory = `{{range $index, $host:=.Hosts}}{{$host.HostName}} ansible_ssh_host={{$host.IP}} ansible_ssh_port={{$host.Port}} {{if ne $host.Key ""}}ansible_ssh_private_key_file=key-{{$host.IP}}{{else}}ansible_user={{$host.User}} ansible_ssh_pass={{$host.Password}}{{end}} 
{{end}}
